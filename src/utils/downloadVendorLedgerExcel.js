...
export const downloadVendorLedgerExcel = (ledgerData, vendorName, isFeedCreditor = false) => {
    try {
        ...
        const excelData = ledgerData.map((entry) => {
            ...
            if (isFeedCreditor) {
                return {
                    ...
                    'Amount (Dr)': (isOpening || !isDebit) ? '-' : (Number(entry.amount) || 0),
                    'Amount (Cr)': (isOpening || !isCredit) ? '-' : (Number(entry.amount) || 0),
        ...
        const totalBags = transactionEntries.reduce((sum, entry) => sum + (Number(entry.bags) || 0), 0);
        ...
        if (isFeedCreditor) {
            totals = {
                ...
                'Quantity (Kg)': totalWeight,
                'Rate (per Kg)': '',
                'Amount (Dr)': totalDebit,
                'Amount (Cr)': totalCredit,
