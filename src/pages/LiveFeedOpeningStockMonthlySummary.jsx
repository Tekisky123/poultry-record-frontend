...
export default function LiveFeedOpeningStockMonthlySummary() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            console.error('Error fetching feed opening stocks:', err);
    ...
    const handleMonthClick = (month) => {
        navigate(`/feed-opening-stock/daily-summary?year=${month.year}&month=${month.monthIndex + 1}`);
    };
    ...
    const handleExportToExcel = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Feed Opening Stock');
        XLSX.writeFile(wb, `Feed_Opening_Stock_${year}-${year + 1}.xlsx`);
    };

    if (loading && !stocks.length) return <div className="flex justify-center p-12"><Loader2 className="animate-spin w-8 h-8 text-yellow-600" /></div>;
                ...
                <div>
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-2">
                            ...
                            Feed Opening Stock (Monthly)
                        </h1>
                    </div>
                    <p className="text-gray-600 mt-1">Monthly Opening Records for Feed Stock</p>
                </div>
                <div className="flex gap-3 mt-4 sm:mt-0 items-center">
                    <select
                        value={year}
                        onChange={(e) => setYear(Number(e.target.value))}
                        className="p-2 border border-yellow-200 bg-yellow-50 text-yellow-800 rounded-md font-semibold focus:ring-yellow-500 focus:border-yellow-500 shadow-sm"
                    >
                        {[0, 1, 2, 3, 4].map(i => {
                ...
                <table className="w-full text-sm text-left">
                    <thead className="bg-gray-100 text-gray-700 uppercase font-semibold border-b-2 border-gray-300">
                        <tr>
                            ...
                            <th className="py-3 px-4 text-right border-r border-gray-300 bg-yellow-50 text-yellow-800">Quantity (kg)</th>
                            <th className="py-3 px-4 text-right border-r border-gray-300 bg-yellow-50 text-yellow-800">Rate</th>
                            <th className="py-3 px-4 text-right bg-yellow-50 text-yellow-800">Amount</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200">
                        {monthlyData.length > 0 ? monthlyData.map(month => (
                            ...
                            >
                                ...
                                <td className="py-3 px-4 text-right text-yellow-700 font-medium border-r">
                            ...
                            <tr>
                                <td colSpan="4" className="py-8 text-center text-gray-500 italic">No records found for FY {year}-{year + 1}</td>
                            </tr>
                        )}
                    </tbody>
                    <tfoot className="bg-gray-100 font-bold text-gray-900 border-t-2 border-gray-400">
                        <tr>
                            ...
                            <td className="py-3 px-4 text-right text-yellow-700 border-r">{totalWeight.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                            ...
                            <td className="py-3 px-4 text-right text-yellow-700">{totalAmount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
