...
export default function CustomerMonthlySummary() {
    ...
    const fetchMonthlySummary = async () => {
        try {
            setLoading(true);
            setError('');
            // Use ledger controller endpoint which handles customers too
            const response = await api.get(`/ledger/${id}/monthly-summary`, { params: { year, type: 'customer' } });
    ...
    const handleExportToExcel = () => {
        ...
        const exportData = data.months.map(m => ({
            Month: m.name,
            'Total Debit (Sold)': m.debit,
            'Total Credit (Paid)': m.credit,
        ...
        exportData.push({
            Month: 'Grand Total',
            'Total Debit (Sold)': data.totals.debit,
            'Total Credit (Paid)': data.totals.credit,
            'Closing Balance': ''
        });
                    ...
                    <table className="w-full text-sm">
                        <thead className="bg-gray-50 border-b border-gray-200">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-right font-medium text-gray-700">Debit (Sold)</th>
                                <th className="px-6 py-3 text-right font-medium text-gray-700">Credit (Paid)</th>
